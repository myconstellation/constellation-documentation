---
ID: 2246
post_title: 'Instance de package, package, versioning, multi-instances et r&eacute;solution des packages'
...
published: true
post_modified: 2016-12-19 09:41:31
---
Un package est une application avec ou sans une interface graphique qu’on déploie sur une sentinelle sur laquelle il sera lancé (<a href="/concepts/architecture-constellation-sentinel-package/#Les_packages">plus d’information ici</a>).
...
<pre class="lang:xml decode:true">&lt;sentinel name="SENTINEL-DEMO" credential="Standard"&gt;
    ...
    &lt;/packages&gt;
&lt;/sentinel&gt;</pre>
L’attribut “name” du package définit en fait le <strong>nom de l’instance</strong> du package. On parlera alors de l’instance “SENTINEL-DEMO/HWMonitor” pour identifier de manière unique l’instance de ce package dans la Constellation.
...
Dans notre cas, Constellation va analyser chaque package de son repository, c’est à dire chaque fichier .ZIP, pour récupérer celui qui a dans son <a href="/concepts/package-manifest/">manifeste</a> (PackageInfo.xml) déclaré la propriété “Name” à “HWMonitor”. Si il ne trouve pas, il va tenter de récupérer le package par le nom “HWMonitor.zip”.
<h3>Versioning des packages</h3>
En plus du nom du package, le <a href="/concepts/package-manifest/">manifeste</a> (PackageInfo.xml) déclare également d’autre information comme la version du package.
...
Ainsi pour résumer, <strong>Constellation utilise donc en priorité le fichier défini par l’attribut “filename” si une valeur est défini pour une instance</strong>. Dans le cas contraire on applique la règle implicite définit ci-dessus.
...
Les instances “SENTINEL-DEMO/HWMonitor1” et “SENTINEL-DEMO/HWMonitor2” sont deux instances différentes dans votre Constellation et peuvent avoir des configuration (settings) différents mais sont basées sur le même code, car elles partagent la même source (le fichier “HWMonitor.zip”).
...
Pour mettre à jour ces deux instances (“SENTINEL-DEMO/HWMonitor1” et “SENTINEL-DEMO/HWMonitor2”) vers une version plus récente, disons la version 1.2, il faudra modifier à la fois l’attribut “filename” de l’instance “HWMonitor1” mais également celle de “HWMonitor2”.
