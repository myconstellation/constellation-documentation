...
Ou si vous souhaitez utiliser le framework AngularJS :
<pre class="lang:html5 decode:true">&lt;script type="text/javascript" src="//code.jquery.com/jquery-2.2.4.min.js"&gt;&lt;/script&gt;
&lt;script type="text/javascript" src="https://ajax.aspnetcdn.com/ajax/signalr/jquery.signalr-2.2.1.min.js"&gt;&lt;/script&gt;
&lt;script type="text/javascript" src="//cdn.myconstellation.io/js/Constellation-1.8.1.min.js"&gt;&lt;/script&gt;
...
requestSentinelUpdates / requestSentinelsList / onUpdateSentinelsList /onUpdateSentinel

Vous disposez de deux méthodes :
<ul>
 	<li>requestSentinelsList() : récupère la liste des sentinelles de votre Constellation</li>
 	<li>requestSentinelUpdates() : crée un abonnement permettant à votre page d’être notifier à chaque mise à jour des sentinelles de votre Constellation (ajout ou suppression de sentinelles, connexion ou déconnexion des sentinelles, mise à jour des informations, etc…).</li>
</ul>
Ces deux méthodes sont de type “void”. La réponse est passer aux handlers respectivement nommés ”onUpdateSentinelsList” et “onUpdateSentinel”.
<ul>
 	<li>Avec l’API JS :</li>
</ul>
...
Lors de la connexion, abonnons-nous mises à jour des sentinelles afin de maintenir notre page à jour :
...
Reprenons notre page ci-dessous et ajoutons la liste des packages pour chaque sentinelle de notre Constellation.
...
La liste de package récupérée ci-dessus n’est pas maintenu à jour. Pour recevoir les mises à jour de vos packages afin d’être notifier à chaque changement d’état vous pouvez ajouter le handler “onReportPackageState”.
...
Dans le code ci-dessous, on ajoute dans chaque objet “Package” la valeur de la consommation du CPU et de RAM.

Ainsi je peux enrichir mon template HTML :
<pre class="lang:html5 decode:true">&lt;ul&gt;
  &lt;li ng-repeat="s in sentinels"&gt;{{s.Description.SentinelName}} [{{s.IsConnected ? "CONNECTED" : "DISCONNECTED"}}] ({{s.Description.OSVersion}})
    &lt;ul&gt;
...
<ul>
 	...
 	<li><em>reload(sentinelName, packageName)</em> : arrête u package, force la sentinelle à télécharger la dernier version du package sur le serveur et redémarre le package</li>
...
Dans notre template ajoutons ensuite les boutons avec des “ng-click” :
...
Vous pouvez rechercher la configuration Constellation avec la méthode “reloadServerConfiguration” :
<ul>
 	<li>Avec l’API JS :</li>
</ul>
...
Cette méthode attend un booléen (deployConfiguration) pour indiquer si oui ou non la configuration est déployée après son rechargement. Dans le cas d’un déploiement, la nouvelle configuration est appliquée aux sentinelles et aux packages de votre sentinelle.